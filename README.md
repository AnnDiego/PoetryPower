...
- Weather: two Open-Meteo points. **Toast / drawer / Notion** use inland home zip 92128 / Rancho Bernardo (hourly at pull hour + daily sunrise). **Morning Toast → X** captions use downtown / coast (`32.7157, -117.1611`) for a Mon–Fri or Fri–Su high/low + condition strip only — it does not seed the haiku. Ann’s drawer tree maps sky + moisture + light — not the daily high. Chili is never raised by temperature. The toast report names the drawer, reason, tell, and both locations.
...
Imagine templates (replace `{HAIKU}` only) live in `scripts/haiku_toast/style_catalog.py`. Enabled templates require a three-line haiku and letters that follow the crumb as Maillard browning, not printed ink.
